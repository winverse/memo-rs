...
pub fn main() {
    ...
    // for mut x in numbers.iter_mut() {
    //     *x *= 3
    // }

    let transformed = numbers.iter_mut().map(|x| *x * 3).collect::<Vec<_>>();
    println!("{:?}", transformed); // should print [3, 6, 9, 12]
